...
def combine_files(data, file_directory):
    ...
    for doc in data:
        if len(doc.get("Files")) == 0:
            ...
            if volume in {"V10", "V11", "V12", "V13", "V14", "V15", "V16"}:
                old_filepath = os.path.join(file_directory, doc["Files"][0].split('\\', 1)[1])
                file_ext = doc["Files"][0].split('.')[1]
                new_filepath = os.path.join(file_directory, "output_files", volume,
                                            doc["Document Handle"] + '.' + file_ext)
                try:
            ...
            if output_volume in {"V10", "V11", "V12", "V13", "V14", "V15", "V16"}:
                ...
                for file in doc.get("Files"):
                    ...
                    # FOR NOW, only working with V10-16 as test data
                    if volume in {"V10", "V11", "V12", "V13", "V14", "V15", "V16"}:
                        old_filepath = os.path.join(file_directory, file.split('\\', 1)[1])
                        try:
                ...
                new_filepath = os.path.join(file_directory, "output_files", output_volume,
                                            doc["Document Handle"] + '.pdf')
                try:
